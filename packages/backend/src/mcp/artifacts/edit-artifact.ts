...
export function getEditArtifactToolConfig(
  agentId: string,
  ...
  const handler: ToolHandler<typeof inputSchema> = async ({
    ...
    try {
      ...
      if (metadata.contentType !== ARTIFACT_CONTENT_TYPE.TEXT || typeof existingContent !== "string") {
        throw new Error(
          `edit_artifact only supports TEXT artifacts (${metadata.filename} is ${metadata.contentType}). Use write_artifact to replace the file.`
        );
      }
  ...
  const config: McpToolConfig<typeof inputSchema> = {
    ...
    description:
      "Surgically modify a TEXT artifact you own by inserting or replacing a range of lines. Use write_artifact to replace the full file or to write non-TEXT content.",
    inputSchema,
    handler,
  };
