 ...
 *
 * Renders a row of tab buttons with icons, labels, and an animated
 * underline indicator powered by framer-motion layoutId.
 *
 * Tabs can optionally be split buttons with a dropdown chevron connected
 * to the context menu system.
 */
...
export function TabBar<T extends string>({
  ...
  const activeText = "text-text-primary";
  const inactiveText = "text-text-muted hover:text-text-secondary";
      ...
      {tabs.map((tab, index) => {
        ...
        return (
          <div key={tab.id} className="flex items-center gap-1">
            ...
            {isSplit ? (
              <div className={`relative flex items-center rounded ${isActive ? "bg-bg-secondary" : ""}`}>
                {/* Main tab button */}
                <button
                  type="button"
              ...
              <button
                type="button"
                className={`relative ${textStyle} rounded ${isActive ? `bg-bg-secondary ${activeText}` : inactiveText}`}
                onClick={() => onTabChange(tab.id)}
              >
                <Icon className="h-3 w-3" />
                <span>{tab.label}</span>
