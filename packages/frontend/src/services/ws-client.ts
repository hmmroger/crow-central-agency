...
/**
 * Build the WebSocket URL based on the current page location, with optional access key.
 * The access key is passed as a query param because the browser WebSocket API does not
 * support custom headers on the Upgrade request.
 */
export function buildWsUrl(accessKey?: string): string {
  const protocol = window.location.protocol === "https:" ? "wss:" : "ws:";
  const base = `${protocol}//${window.location.host}/ws`;

  if (accessKey) {
    return `${base}?accessKey=${encodeURIComponent(accessKey)}`;
  }
